...
$app->post('/api/login', function (Request $request, Response $response) {
    ...
    $sql = "SELECT username, admin FROM users where UPPER(username) = :username and password = :password";
    ...
    try {
        ...
        if (count($foundusers) == 1) {
            ...
            $admin = intval ($foundusers[0]['admin']);
            //create a jwt token
            ...
            $token = $jwt->_jwt_encode_data(
                ...
                array("username" => $username, "admin" => $admin) // create an array if all user infos 
            );
